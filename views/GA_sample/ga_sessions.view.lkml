view: ga_sessions_sample {
  ...
  dimension_group: data {
    ...
    sql: TIMESTAMP(DATE_ADD(PARSE_DATE('%Y%m%d', _TABLE_SUFFIX), INTERVAL DATE_DIFF(CURRENT_DATE(),DATE('2017-08-02'),DAY) DAY)) ;;
  }
  ...
  dimension: geo_network__loaction {
    type: location
    group_label: "Geo Network"
    group_item_label: "Location"
    sql_latitude: ${geo_network__latitude} ;;
    sql_longitude: ${geo_network__longitude} ;;
  }
  ...
  dimension: session_duration {
    ...
    type: tier
    tiers: [10, 30, 60, 120, 300, 600]
    style: integer
    sql: ${totals__time_on_site} ;;
  }

  dimension: totals__total_transaction_revenue {
    type: number
    sql: ${TABLE}.totals.totalTransactionRevenue*1e-6 ;;
    group_label: "Totals"
  ...
  dimension: totals__transaction_revenue {
    type: number
    sql: ${TABLE}.totals.transactionRevenue*1e-6 ;;
    group_label: "Totals"
  ...
  dimension: unique_session_id {
    primary_key: yes
    type: string
    sql: CONCAT(${full_visitor_id}, ' | ', ${visit_id}, ' | ', ${date}) ;;
  }
  ...
  dimension: session_with_purchase {
    type: yesno
    sql: ${totals__transactions} IS NOT NULL ;;
  }

  dimension: abandonment {
    type: number
    sql: IF(DATE_DIFF(CURRENT_DATE(),${data_date}, DAY) > 7,1,0) ;;
  }
  ...
  measure: sessions_with_purchase {
    type: count_distinct
    sql: ${session_id} ;;
    filters: [session_with_purchase: "yes"]
  }

  measure: ecommerce_conversion_rate {
    type: number
    value_format_name: percent_1
    sql: ${sessions_with_purchase}/${total_sessions};;
  }

  measure: commercial_conversion_rate {
    type: number
    value_format_name: percent_1
    sql: SUM(IF(${totals__pageviews}>=3,1,0))/${total_sessions};;
  }

  measure: transactional_conversion_rate {
    type: number
    value_format_name: percent_1
    sql: SUM(${totals__transactions})/${total_sessions};;
  }
...
view: ga_sessions_sample__hits {
  ...
  dimension: e_commerce_action__action_type {
    type: number
    sql: CAST(${TABLE}.eCommerceAction.action_type AS INT64) ;;
  ...
  dimension: ga_sessions_sample__hits {
    type: string
    hidden: yes
    sql: ga_sessions_sample__hits ;;
  }
  ...
  dimension: transaction__transaction_revenue {
    type: number
    sql: ${TABLE}.transaction.transactionRevenue*1E-6 ;;
    group_label: "Transaction"
    group_item_label: "Transaction Revenue"
  }
...
view: ga_sessions_sample__custom_dimensions {
  dimension: ga_sessions_sample__custom_dimensions {
    type: string
    hidden: yes
    sql: ga_sessions_sample__custom_dimensions ;;
  }
...
view: ga_sessions_sample__hits__product {
  dimension: custom_dimensions {
    hidden: yes
    sql: ${TABLE}.custom Dimensions ;;
  }

  dimension: custom_metrics {
    hidden: yes
    sql: ${TABLE}.custom Metrics ;;
  }
  ...
  dimension: local_product_revenue {
    type: number
    sql: ${TABLE}.localProductRevenue*1E-6 ;;
  }
...
view: ga_sessions_sample__hits__promotion {
...
view: ga_sessions_sample__hits__custom_metrics {
  dimension: index {
    type: number
    sql: ${TABLE}.index ;;
  }
...
view: ga_sessions_sample__hits__custom_variables {
...
view: ga_sessions_sample__hits__custom_dimensions {
  dimension: index {
    type: number
    sql: ${TABLE}.index ;;
  }
...
view: ga_sessions_sample__hits__experiment {
...
view: ga_sessions_sample__hits__publisher_infos {
...
view: ga_sessions_sample__hits__product__custom_metrics {
  dimension: index {
    type: number
    sql: ${TABLE}.index ;;
  }
...
view: ga_sessions_sample__hits__product__custom_dimensions {
  dimension: index {
    type: number
    sql: ${TABLE}.index ;;
  }
